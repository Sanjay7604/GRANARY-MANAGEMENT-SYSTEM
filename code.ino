...
const char *ssid = "ESP32-AP";  // SSID of the ESP32 soft AP
...
void loop() {
  ...
  lightDetected = digitalRead(IR_SENSOR_PIN);
...
void handleRoot() {
  ...
  bool lightDetected = digitalRead(IR_SENSOR_PIN);

  // Check if smoke level exceeds threshold
  bool smokeDetected = (smokeLevel > 150);
...
webpage += "<p>Light Detected: " + String(lightDetected ? "No" : "Yes") + "</p>";
